...
const addComment = asyncHandler(async(req, res)=>{
    ...
    if(!content){
        throw new ApiError(400, "Content is unavailable")
    }

    if(!isValidObjectId(videoId)){
        throw new ApiError("Video Id invalid")
    }
    ...
    if(!comment){
        throw new ApiError(500, "Comment failed !! Try again")
    }
...
const deleteComment= asyncHandler(async(req,res)=>{
    ...
    if(comment?.owner.toString()!==req.user?.id.toString()){
        throw new ApiError(400, "Unable to do Unauthorized changes")
    }
